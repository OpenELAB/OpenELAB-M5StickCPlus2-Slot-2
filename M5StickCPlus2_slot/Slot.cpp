...
void Slot::draw() {//Key function to draw on slot machine icons
	...
	while (y < height) {
		...
		if (y < shadowHeight || y >= height - shadowHeight) {
			...
			shadowY = 0;
			M5.Lcd.pushImage(x, y + PAD_Y, SYM_WIDTH, h, data);//plot
		}
		offset += h;
		y += h;
	}
...
void Slot::start(int _acc, int _maxVel) {//Launch Slot Machines
	...
	maxVel = _maxVel;
	tick = millis();
	state = SLOT_START;//Setting Slot Machine Status
	index = -1;
}

void Slot::stop(int _acc, int _minVel) {//Stop a line of slot machines
	acc = _acc;
	minVel = _minVel;
	state = SLOT_STOP;
}

bool Slot::update() {//Update Slot Machine Current Display
